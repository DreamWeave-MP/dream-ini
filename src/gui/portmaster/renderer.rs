...
fn solid_fan_run_for_center<'a>(
    ...
    let Some(color) = solid_fan_triangle_color(first, texture, clip) else {
        return Ok(None);
    };

    let mut triangles = vec![first.vertices];
    let Some(expected_area_sign) = fan_triangle_area_sign(first) else {
        return Ok(None);
    };
    let mut used_boundaries = vec![previous_boundary, current_boundary];
    ...
    while offset + 2 < mesh.indices.len() {
        ...
        if used_boundaries
            .iter()
            .any(|boundary| same_fan_vertex(*boundary, next_boundary))
        {
            break;
        }
        if solid_fan_triangle_color(candidate, texture, clip) != Some(color) {
            break;
        }
        ...
        triangles.push(candidate.vertices);
        previous_boundary = current_boundary;
        current_boundary = next_boundary;
        used_boundaries.push(next_boundary);
        offset += 3;
    }

    if triangles.len() < SOLID_FAN_MIN_TRIANGLES
        || same_fan_vertex(previous_boundary, current_boundary)
    {
        return Ok(None);
    }
    let mut polygon: Vec<_> = used_boundaries
        .iter()
        .map(|boundary| boundary.vertex)
        .collect();
    polygon.push(center_vertex);
