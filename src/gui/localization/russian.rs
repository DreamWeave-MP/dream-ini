...
pub(super) const fn text(key: UiText) -> &'static str {
    match key {
        ...
        UiText::ControllerHelp => {
            "Контроллер: D-pad/левый стик — перемещение • A — переключить/выбрать • X — очистить выбранный путь • Start — импорт, когда всё готово • Select — выйти • влево/вправо — изменить параметры • правый стик — прокрутка cfg • LB/RB — страницы generated cfg"
        }
        UiText::PickerControllerHelp => {
            "Контроллер: D-pad/левый стик — перемещение • A/Enter — открыть или выбрать • B/Влево — родительский каталог • Вправо — войти • Start — выбрать текущий/ожидаемый путь • Select — отмена • LB — переключить скрытые каталоги"
        }
