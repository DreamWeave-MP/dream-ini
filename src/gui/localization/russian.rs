...
pub(super) const fn text(key: UiText) -> &'static str {
    match key {
        ...
        UiText::ControllerHelp => {
            "Контроллер: D-pad/левый стик — перемещение • A — переключить/выбрать • B — выйти • X — очистить выбранный путь • Start — импорт • влево/вправо — изменить параметры • правый стик — прокрутка cfg"
        }
        UiText::PickerControllerHelp => {
            "Контроллер: D-pad/левый стик — перемещение • A/Enter — открыть или выбрать • B — отмена • Влево — родительский каталог • Вправо — войти • Start — выбрать текущий путь • LB — скрытые каталоги"
        }
    }
}
