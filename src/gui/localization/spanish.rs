...
pub(super) const fn text(key: UiText) -> &'static str {
    match key {
        ...
        UiText::ControllerHelp => {
            "Mando: cruceta/stick izquierdo mueve • A alterna/elige • X borra la ruta seleccionada • Start importa cuando esté listo • Select sale • izquierda/derecha ajusta opciones • stick derecho desplaza el cfg generado • LB/RB pagina el cfg generado"
        }
        UiText::PickerControllerHelp => {
            "Mando: cruceta/stick izquierdo mueve • A/Enter abre o elige • B/Izquierda va al directorio superior • Derecha entra • Start elige la ruta actual/esperada • Select cancela • LB alterna directorios ocultos"
        }
