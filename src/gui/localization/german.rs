...
pub(super) const fn text(key: UiText) -> &'static str {
    match key {
        ...
        UiText::ControllerHelp => {
            "Controller: Steuerkreuz/linker Stick bewegt • A schaltet/wählt • B beendet • X leert ausgewählten Pfad • Start importiert • links/rechts passt Optionen an • rechter Stick scrollt die generierte cfg"
        }
        UiText::PickerControllerHelp => {
            "Controller: Steuerkreuz/linker Stick bewegt • A/Enter öffnet oder wählt • B bricht ab • Links geht zum übergeordneten Verzeichnis • Rechts öffnet • Start wählt den aktuellen Pfad • LB schaltet versteckte Verzeichnisse um"
        }
    }
}
